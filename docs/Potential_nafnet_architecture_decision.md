...
- **Domain confirmed as semiconductor SEM imagery** — likely a via/contact/pillar structure.
  ...
  **hallucinated detail actively harmful**, not just undesirable — ruling out adversarial
  (GAN) sharpening approaches. (note: went through more of the dataset and their are all kinds 
  of images present, not just SEM imagery)
...
- **Building a custom architecture:** only worth doing if absolutely needed or if it truly provides
  a competitive edge considering the short deadline.
